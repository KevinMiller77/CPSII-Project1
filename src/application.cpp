...
namespace Keys
{
    ...
    // This is defined inside the cpp fiel just so it only gets compiled once
    ...
    // Templated function to grab whatever type of key we want
    template <typename T>
    ...
    // Checks if the input is a valid keyword
    // as defined above and returns either a KeyType
...
std::vector<std::string>* Application::WordsFromInput()
{
    std::string input;  //""
...
Seat* Application::getBestSeat(uint32_t credits, SeatType seatIN, CarType carIN, CarColor colorIN)
{
...
Seat* Application::getBestSeat(uint32_t credits, SeatType seatIN, CarType carIN)
{
...
int Application::GetPersonFromName(std::string& name)
{   
    //to lower input
...
bool Application::Destroy(uint32_t reservationNumber)
{
